...
fetch('../json/supplements.JSON')
  ...
  .then(data => {
    ...
    let mokamel = ''
    data.supplements.forEach((val, index) => {
      ...
      mokamel += `
         ...
         <div class="card-footer bg-transparent">
            <button onclick="addToCartByIndex('supplements', ${index})" class="btn btn-primary w-100">Add to Cart</button>
         </div>
          </div>
          </div>
            `
