const listVasayelGym = document.querySelector(".Sportswear-gym")



fetch('/code/json/Sportswear.json')
  .then(Response => Response.json())
  .then(data => {
    products.data = data
    let vasayelGym = '';

    products.data.Sportswear.forEach((val, index) => {

      vasayelGym += `<div class="col-lg-3 col-md-4 col-6">
    ...
    listVasayelGym.innerHTML = vasayelGym;
    ...
    cartList += ` <li class="list-group-item d-flex justify-content-between align-items-center">
            ...
            <div>
                <button class="btn btn-outline-secondary btn-sm me-1" onclick="increase(${index})">+</button>
                 <span style="font-weight: 800;" > ${val.quantity}</span>
...
function addToCart(index) {
  const product = products.data.Sportswear[index];
  const existing = cart.find(val => val.name === product.name && val.caption === product.caption && val.price === product.price);
  if (existing) {
    existing.quantity += 1;
  }
  else {
